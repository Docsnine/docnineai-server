...
export function exportToPDF(res, { meta, output, stats, securityScore }) {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="${meta.name || "docs"}-documentation.pdf"`);
  doc.pipe(res);
  ...
  doc.fill(DARK).fontSize(11).font("Helvetica")
     .text(`Generated by Project Documentor  •  ${new Date().toLocaleDateString()}`, 50, 155);
  ...
  statItems.forEach(([label, val]) => {
    doc.fill(BLUE).fontSize(20).font("Helvetica-Bold").text(String(val), sx, doc.y, { width: 110, align: "center" });
    doc.fill(MUTED).fontSize(9).font("Helvetica").text(label, sx, doc.y, { width: 110, align: "center" });
    sx += 120;
  });
  ...
  const heading1 = (text) => {
    ...
    doc.moveDown(0.5).fill(DARK).fontSize(20).font("Helvetica-Bold").text(text, 50);
    doc.fill(MUTED).moveTo(50, doc.y).lineTo(550, doc.y).stroke().moveDown(0.5);
  };

  const heading2 = (text) => {
    doc.moveDown(0.5).fill(BLUE).fontSize(13).font("Helvetica-Bold").text(text, 50);
    doc.fill(DARK);
  };
  ...
  const codeBlock = (text) => {
    ...
    doc.rect(50, doc.y, PAGE_W, 12 + safe.split("\n").length * 11).fill("#F1F5F9");
    doc.fill("#0F172A").fontSize(8).font("Courier")
  ...
  if (output.securityReport) {
    ...
    const scoreColor = (securityScore || 0) >= 75 ? GREEN : (securityScore || 0) >= 50 ? "#D97706" : RED;
...
export async function exportToNotion({ output, meta, stats, securityScore }) {
  if (!process.env.NOTION_API_KEY || !process.env.NOTION_PARENT_PAGE_ID) {
    throw new Error("NOTION_API_KEY and NOTION_PARENT_PAGE_ID must be set in .env");
  }
  ...
  const mainPage = await notion.pages.create({
    ...
    properties: {
      title: { title: [{ text: { content: `📄 ${meta.name || "Project"} — Documentation` } }] },
    },
    children  : [
      ...
      notionParagraph(`Files: ${stats?.filesAnalysed} | Endpoints: ${stats?.endpoints} | Models: ${stats?.models} | Security Score: ${securityScore}/100`),
    ],
  });
...
function notionCode(text) {
  return { object: "block", type: "code",
    code: { rich_text: [{ text: { content: text.slice(0, 2000) } }], language: "markdown" } };
}

// Convert markdown sections → Notion blocks (simplified)
function markdownToNotionBlocks(markdown) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("# "))   { blocks.push(notionHeading(line.slice(2), 1)); continue; }
    if (line.startsWith("## "))  { blocks.push(notionHeading(line.slice(3), 2)); continue; }
    if (line.startsWith("### ")) { blocks.push(notionHeading(line.slice(4), 3)); continue; }
    if (line.startsWith("---"))  { blocks.push(notionDivider()); continue; }
